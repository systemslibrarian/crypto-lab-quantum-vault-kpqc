...
## What It Is
Quantum Vault is a browser demo that combines AES-256-GCM, Shamir SSS / GF(2⁸), SMAUG-T (스마우그-T) KEM, and HAETAE (해태) signature in one sealing/opening pipeline. It is built to show how a secret can be encrypted once, split into threshold shares, and opened only when enough valid shares are recovered. SMAUG-T handles post-quantum key encapsulation for per-share wrapping, and HAETAE verifies container integrity before decryption. The security model is hybrid: symmetric encryption plus post-quantum asymmetric primitives with threshold reconstruction. A plain-language **"How this box works"** walkthrough tells the four-primitive envelope story in order, per-step narration lights up as each pipeline stage runs, and a Shamir key-strip visualization shows the real 32-byte AES key splitting into 3 shares and re-forming from any 2 (or landing on unrelated bytes from just 1). On open, the recovered per-keyholder share strips animate combining into the rebuilt key, which is placed directly beneath the true original and compared byte-for-byte — matching cells earn a green ✓, so "any 2 rebuild it exactly" is a seen fact, not a caption; with only 1 share the original is left unshown because one share reveals nothing about it, which the demo names as the security guarantee. An "inside step 3" mini-diagram splits the SMAUG-T lock into its two real layers (password → PBKDF2 → secret key, then KEM encapsulate/decapsulate → wrapping key → share), and a small 2-D lattice sketch grounds why the post-quantum layer resists quantum attack. Inline glossary tooltips introduce KEM, Shamir/GF(2⁸), "lattice-based", and sign-vs-verify for newcomers; a classical-vs-post-quantum toggle and a one-click tamper demo make the "harvest now, decrypt later" motivation and the confidentiality-vs-integrity failure distinction concrete; expert-facing "when to use each primitive" reference cards are gated behind a **Go deeper** reveal so the first-scroll path stays focused on the interactive core.
...
4. **Shamir key-strip, side by side** — the genuine AES key breaking into 3 keyholder-colored share strips on seal; on open the recovered shares animate combining, and the rebuilt key sits beneath the true original with a per-cell green ✓ where the bytes match (or, below threshold, no original is shown because one share cannot reveal it — the security guarantee made explicit).
5. **Inside step 3: two nested locks** — a mini-diagram separating what your password does (PBKDF2 → unlock the keyholder's SMAUG-T secret key) from what the KEM does (encapsulate/decapsulate → recover the wrapping key that unlocks the share), so the post-quantum layer is distinct from the password layer.
6. **2-D lattice sketch** — a tiny grid where the nearest lattice point is trivially visible, captioned to explain why the same "nearest point" search (Module-LWE) is intractable in the hundreds of dimensions SMAUG-T uses.
7. **Glossary tooltips** — click-to-expand newcomer definitions with one-line analogies for KEM, Shamir sharing / GF(2⁸), lattice-based hardness, and signature vs. verify.
8. **Classical vs. post-quantum** — a toggle tied to the SMAUG-T step showing what a future quantum attacker does to RSA/ECDH ("harvest now, decrypt later") versus why the lattice KEM resists it.
9. **Two failure modes, distinctly** — a wrong password turns just that keyholder's SMAUG-T pill amber ("share unavailable — N of 2 needed"); a tamper demo button flips a ciphertext byte and turns the HAETAE-verify pill red, stopping before any share is tried.
10. **Go deeper (gated reference)** — expert-facing "when to use each primitive / what breaks if you don't" cards revealed on demand, keeping the first scroll on try-a-box → watch-the-pipeline → read-the-story.
11. **Vault management** — export/import individual `.qvault` containers and full vault state, reset to demo, and an EN/한국어 language toggle.
