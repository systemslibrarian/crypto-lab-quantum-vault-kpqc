...
const strings: Record<string, { en: string; ko: string }> = {
  ...
  ksOpenCaptionOk:   { en: 'These 2 recovered shares combine back into the key. Compare it to the original below — every cell carries a green ✓ because the bytes are identical:',
                       ko: '되찾은 이 2개 조각이 키로 다시 결합됩니다. 아래 원본과 비교해 보세요 — 바이트가 동일하므로 모든 칸에 녹색 ✓가 표시됩니다:' },
  ksOpenCaptionBad:  { en: '1 share alone lands on an unrelated key — its bytes are scrambled garbage:',
                       ko: '조각 1개만으로는 무관한 키가 됩니다 — 바이트가 뒤섞인 쓰레기 값입니다:' },
  ...
  ksOriginalLabel:   { en: 'original key',              ko: '원본 키' },
  ksCompareTo:       { en: '↓ does the rebuilt key match, byte for byte?',
                       ko: '↓ 복원된 키가 바이트 단위로 일치할까요?' },
  ksNoOriginal:      { en: 'With only one share there is nothing to compare against: a single share reveals NOTHING about the real key, so its true bytes stay unknown. That unknowability is the whole point of the 2-of-3 threshold.',
                       ko: '조각이 하나뿐이면 비교할 대상이 없습니다. 조각 하나는 실제 키에 대해 아무것도 드러내지 않으므로 진짜 바이트는 알 수 없습니다. 이 알 수 없음이 바로 3개 중 2개 임계값의 핵심입니다.' },
