 ...
 *    an unknown backdrop, or a `color-mix()`. `src/styles/vault.css` uses no
 *    `color-mix()` at all, but `index.html`'s inline shared-top-bar stylesheet
 *    uses four of them, and they carry the bar's entire ink and edge: `--cl-ink`
 *    is `color-mix(in srgb, var(--accent) 60%, #eafff8)` and is the colour of
 *    the CL badge and of every `.cl-btn` label (Menu, GitHub, EN, 한국어, the
 *    theme toggle); `.cl-btn`'s border and `.cl-btn:hover`'s fill are two more.
 *    axe resolves none of them and files them under `incomplete`, so a
 *    violations-only gate measured nothing about the bar this lab ships on every
 *    page. This lab also sets `--accent:#003478` — the Taegukgi navy, a DARK
 *    hue — inline on the bar, which is precisely the case that mix exists for.
 *  - text faded by an ancestor's `opacity` — axe reads the declared `color`,
 *    which is not the colour that lands on screen. This page has three such
 *    rules: `.btn-outline:disabled { opacity: 0.5 }` (exempt, an inactive
 *    component), the `.ks-converge` / `.ks-diverge` share-merge classes at 0.35
 *    and 0.4, and `.keystrip { opacity: 0 }` — the Shamir key-strip mounts
 *    invisible and is faded in by a `ks-in` class added on the next animation
 *    frame. That last one is why the walk cannot simply trust a declared
 *    colour, and why `expectNotBlank` in `gate.ts` measures the reduced-motion
 *    end state separately. A fourth, `.step-sub { opacity: 0.95 }` on the
 *    per-pill sub-labels, was DELETED rather than measured around: it duplicated
 *    a de-emphasis the smaller type already carried, and it dragged the pending
 *    pill's sub-label from 4.86:1 to 4.40:1 — under the floor, for nothing.
 *
 ...
 *
 * Three things this fleet's pages forced on top of that, each of which
 ...
 *    clipping boxes are `.panel-area` (`max-height: 0; overflow: hidden` until
 *    the `open` class lands — the whole deposit/retrieve form lives in it),
 *    `.table-scroll` (`overflow-x: auto`, which wraps the demo-credentials
 *    table), and `.ks-strip` (`overflow: hidden` around the 16 key-byte cells).
 *    Content scrolled past a clip is not dimmed or partly drawn — it is absent,
 ...
 *    has two SVGs and neither contains a `<text>`: the top bar's `aria-hidden`
 *    GitHub mark, and the `role="img"` lattice figure, whose meaning is carried
 *    by an `aria-label` and a visible `.lattice-caption` paragraph beneath it.
 *    The branch is kept because the alternative is a helper that silently
 ...
 * such rules that paint. `.disclaimer-list li::before` draws the `⚠` in front
 * of every disclaimer; `.keyhole::before` and `.keyhole::after` draw the
 * keyhole itself on all nine vault boxes out of a border-radius ring and a
 * `currentColor` block, with `content: ''` — shapes, not glyphs. `nontext.ts`
 * measures generated content directly and runs at every driven state, so these
 * are gated rather than hand-measured. The keyhole is inside an
 * `aria-hidden="true"` element and is decorative: every box states "OCCUPIED"
 * or "EMPTY" in text of its own, and the box's `aria-label` says the same.
 */
 ...
 * uses. The parameter is kept because the narrowed form is what makes a
 * short-lived state measurable at all: a class that lives 500ms is shorter than
 * a full axe pass, so a whole-page scan would catch it only sometimes, and a
 * sometimes-measured assertion is worse than none. This lab has one such class
 * — `.ks-converge` / `.ks-diverge` on the merging Shamir share strips — and
 * under the reduced motion this gate asserts, `keystrip.ts` returns before
 * adding either, so the transient never exists in a scanned state.
 */
export async function auditContrast(page: Page, within = 'body *'): Promise<ContrastFailure[]> {
  return page.evaluate((rootSelector) => {
     ...
     * ink, its button edges and its hover fill are all `color-mix(in srgb, …)`,
     * and Chromium reports that to `getComputedStyle` unchanged rather than
     ...
     * gradient whose worst stop spans its element. Each gradient is *sampled* at
     ...
     *
     * NEITHER BRANCH RUNS ON THIS PAGE TODAY: `vault.css` and the inline
     * top-bar stylesheet between them declare no `gradient` of any kind — every
     * surface here is a flat colour. Both are kept because they are the shared
     * paint core `nontext.ts` also uses, and because a single decorative wash
     * added to `body` would otherwise be measured as if it were not there.
     */
     ...
     *
     * A driven pass here walks a page several screens deep — the nine-box wall,
     * the open panel's form, the four-pill pipeline, the key-strip rows (one
     * `<span class="ks-cell">` per key byte, up to five rows of sixteen), the
     * how-it-works and decision panels, four glossary disclosures and the
     * failure-mode cards — all re-walking the same ancestors up to `<body>`.
     ...
     * those are `.panel-area` (`max-height: 0; overflow: hidden` until it gains
     * `open`), `.table-scroll` around the demo-credentials table, and
     * `.ks-strip` around the key-byte cells.
     */
    ...
    const svgUnderlay = (el: Element, box: DOMRect): RGBA => {
      ...
      // exactly the shape that guard exists for. This lab's lattice figure is
      // built from <circle> and <line> elements and carries no <text>, so the
      // branch is inert here too — but its <line> IS the stroke-only shape the
      // guard exists for, and a labelled version of that figure would be
      // measured against an invented opaque black rectangle without it.
    ...
    const isVisible = (el: Element): boolean => {
      ...
      //
      // This lab DOES use the idiom, and the guard is load-bearing rather than
      // defensive here: `vault.css` defines `.sr-only` with
      // `position:absolute; width:1px; height:1px; clip: rect(0, 0, 0, 0)`, and
      // `index.html` applies it to `<h2 id="vault-wall-label">Vault — Safety
      // Deposit Boxes</h2>` — the accessible name of the vault section, painted
      // nowhere. Without this test that heading is measured, and the numbers it
      // produces describe ink that was never laid down. The test is deliberately
     ...
     *
     * What this lab hides was enumerated from the source rather than assumed:
     *
     *  - `.keyhole` on all nine vault boxes — pure decoration, drawn entirely
     *    from `::before`/`::after` shapes with `content: ''`. Each box states
     *    "OCCUPIED" or "EMPTY" in exposed text and repeats it in its
     *    `aria-label`.
     *  - `.pipeline-arrow` (`→`) between the four pipeline pills, `.layer-arrow`
     *    (`↓`) between the two nested-lock nodes, `.ks-split-arrow` and `.ks-vs`
     *    between the key-strip rows. All are connectors between items that are
     *    themselves exposed and in DOM order; the arrow adds no fact.
     *  - `.ks-strip` and its `.ks-cell` / `.ks-mark` children — the coloured
     *    byte cells and their per-cell ✓/✗ marks. Colour is the only channel
     *    they carry, so they are hidden and the SAME verdict is stated in text
     *    by `.ks-verdict` and `.ks-caption`, which are exposed and ARE measured.
     *
     * Nothing on this page hides a VALUE that is not also stated in exposed
     * text: no decrypted message, no verdict, no error and no share count is
     * inside an `aria-hidden` subtree — checked rather than assumed.
     */
     ...
     *
     * Neither SVG on this page carries character data: the top bar's GitHub
     * mark is a single `<path>`, and the lattice figure is `<circle>`/`<line>`
     * geometry labelled by an `aria-label` and a visible caption. The guard
     * stays because it costs nothing and because the first labelled SVG figure
     ...
     * `vault.css` sets `html { background: #f0ede6 }` — the warm hanji paper —
     * with an explicit `html[data-theme='dark'] { background: #15130e }`
     * override, so the first branch below returns an opaque colour and the
     * `<body>` fallback is never reached. That is one deleted declaration away
     * from breaking, on a page many screens deep whose dark surface is
     * `#15130e`: fall through to white instead and every element below the first
     * screen would suddenly be measured against a page that does not exist.
     */
