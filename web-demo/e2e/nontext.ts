 ...
 *
 * The gate this replaces had no 1.4.11 oracle of any kind — it ran axe's
 * `violations` array and nothing else, so every control boundary on the page
 * was unmeasured. That matters here more than the count suggests, because this
 * lab paints a lot of controls: nine `.deposit-box` tiles carrying
 * `role="button"`, the two `.pq-btn` mode buttons, four `.btn-vault-action` /
 * `.btn-reset` vault-management buttons, and — inside the panel, which only
 * exists once a box is clicked — `.btn-primary`, `.btn-outline`,
 * `.btn-cancel-x`, `.btn-tamper`, three password inputs and a textarea. A gate
 * that never opened the panel could not have judged the second half of that
 * list even if it had had an oracle.
 *
 ...
 *    the wrong surface, and `escaped` is reported so triage can see it. On this
 *    page the live case is `.disclaimer-list li::before`, the `⚠` in front of
 *    each disclaimer — and it IS absolutely positioned, so its finding is
 *    reported `POSITIONED` and was hand-checked rather than taken on trust.
 *    (The keyhole's two pseudos have `content: ''` and draw shapes rather than
 *    glyphs, so this half skips them; they are decorative and inside an
 *    `aria-hidden` element, and every box states OCCUPIED/EMPTY in text.)
 *
 ...
 * background beneath — because a control's fill is often an `rgba()` over a
 * panel that is itself translucent, and reading the declared value would
 * measure something the page never paints.
 */
...
export async function auditNonText(page: Page, within = 'body *'): Promise<NonTextFailure[]> {
  return page.evaluate((rootSelector) => {
     ...
     *
     * A driven pass here walks the nine-box wall, the open panel's whole form,
     * the pipeline pills and the key-strip rows (one `<span class="ks-cell">`
     * per key byte). Without the caches the pass re-reads the same computed
     ...
     * theme. This lab paints flat colours only (`html { background: #f0ede6 }`,
     * `#15130e` in dark), so the sampler resolves on its first branch — but it
     * borrows `contrast.ts`'s gradient-aware sampler wholesale rather than
     * approximating it a second time, so the two oracles cannot drift apart.
     */
    ...
    const CONTROL = [
      ...
      // missed entirely: the shared header's GitHub control is
      // `<a class="cl-btn cl-btn--label">`, and with no `a` in this list nothing
      // ever judged it. Restricted to link-shaped BUTTONS — prose links identify
